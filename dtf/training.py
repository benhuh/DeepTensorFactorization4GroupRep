...
logging.getLogger("pytorch_lightning").setLevel(
    logging_verbose_level
)  # hide printing of GPU available: True (cuda), used: True
...
def get_trainer(hparams, train_batch, ckpt_path=None):
    ...
    earlystop_callback_dict = {
        "scheduler/counter": EarlyStopping(
            monitor="scheduler/counter",
            min_delta=-1,
            mode="max",
            stopping_threshold=hparams.counter_threshold[-1],
            check_on_train_epoch_end=True,
        ),
        "accuracy/val": EarlyStopping(
            monitor="accuracy/val",
            patience=int(2000 / hparams.val_check_interval),
            min_delta=1e-4,
            mode="max",
            stopping_threshold=99,
            check_on_train_epoch_end=False,
        ),
        "loss/train": EarlyStopping(
            monitor="loss/train",
            min_delta=-1,
            mode="min",
            stopping_threshold=1e-9,
            divergence_threshold=1e2,
            check_on_train_epoch_end=True,
        ),
    }

    earlystop_callback = [
        earlystop_callback_dict[key] for key in [*set(hparams.earlystop)]
    ...
    trainer_args = {
        ...
        "log_every_n_steps": 1,
        "callbacks": [
            TQDMProgressBar(refresh_rate=1, enable_val=hparams.enable_val_progress),
    ...
    if torch.backends.mps.is_available() and len(hparams.gpus) > 0:
        trainer_args.update(
            {
                "accelerator": "mps",
                "devices": hparams.gpus,  # "strategy": "ddp",
            }
        )
...
def add_model_specific_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help=" 0<N<=1 -> fraction of dataset",
    )
    ...
    parser.add_argument(
        "--betas", nargs="+", type=float, default=[0.9, 0.0]
    )  # default=[0.8, 0.9])
    ...
    parser.add_argument(
        "--use_scale", choices=[None, "log", "square", "sqrt", "exp"], default=None
    )  # , 'p1_reg', 'max'          # parser.add_argument("--use_scale", dest="use_scale", action="store_true", default=False) # log-scale for weight decay scheduler
...
def get_hparams(*args, default_kwargs=None, parser=None) -> Namespace:
    ...
    parser.add_argument(
        "--enable_val_progress",
        dest="enable_val_progress",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--profiler", dest="profiler", action="store_true", default=False
    )
...
def get_tensor_width(hparams):
    if hparams.model in ["Deep_Tensor_Net", "Deep_Tensor_Net_Lie", "Transformer"]:
        if getattr(hparams, "tensor_width", None) in [None, 0]:
            hparams.tensor_width = get_default_N(
                hparams.task_name, default_modulus=hparams.modulus
            )
    if hparams.verbose:
        print(
            "Task:", hparams.task_name, "tensor_width", hparams.tensor_width
        )  # , "Total batch:", hparams.total_batch
...
def get_log_name(hparams):  # , print_flag=False):
    ...
    if hparams.task_name in (ALGORITHMIC_OPERATORS | TOY_OPERATORS):
        ...
        if hparams.tensor_width is not None and hparams.tensor_width != 1:
            task += f"_{hparams.tensor_width}"
        if (
            hparams.task_rank is not None
            and ("completion" in hparams.task_name)
            and hparams.task_rank is not None
        ):
    ...
    train_frac_str = "frac"
    train_frac = (
        hparams.train_frac if hparams.train_frac > 1 else hparams.train_frac * 100
    )
...
def get_ckptpath(path):  # , version_limit=None  # Recursive!!
    ...
    ckptpath_list = (
        [path] if path.suffix == ".ckpt" else list(path.glob("*.ckpt"))
    )  # if path.suffix=='.ckpt':   # path.stem /  path.name

    if len(ckptpath_list) == 0:
        sub_path_list = list(
            path.glob("*")
        )  # sub_path_list.sort() #(key=os.path.getmtime)  # sort by the time of last modification
...
class EarlyStopping(EarlyStopping_orig):
    def on_validation_end(
        ...
        if (
            current_step <= trainer.val_check_interval
        ):  # 20: # to avoid the bug in which the validation logging doesn't work for first few epochs.
            return
