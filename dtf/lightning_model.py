...
class LITmodel(LightningModule, Logging_Module):
    ...
    def __init__(self, hparams: Namespace):  # , device) -> None:
        ...
        self.last_logs = {
            criterion: torch_inf for criterion in self.hparams.scheduler_criterion
        }
        self.A_hist = None
    ...
    def scheduler_counter(self):
        ...
        scheduler = (
            schedulers[0] if isinstance(schedulers, (list, tuple)) else schedulers
        )
        return (
            scheduler.scheduler_counter
            if hasattr(scheduler, "scheduler_counter")
            else default_count
        )
    ...
    def scheduler_criterion(self):
        if (
            self.hparams.scheduler_criterion is not None
        ):  # and len(self.hparams.scheduler_criterion)>0:
            assert len(self.hparams.scheduler_criterion) in [1, 2]
            return (
                self.last_logs[self.hparams.scheduler_criterion[0]],
                self.last_logs[self.hparams.scheduler_criterion[-1]],
            )
        else:
            return 0.0, 0.0
    ...
    def configure_optimizers(self) -> Tuple[List[Any], List[Dict]]:
        ...
        if not (
            ...
            param_group_2 = {
                ...
                "lr": 1 * lr,
                "weight_decay": self.hparams.conv_weight_decay,
            }
            ...
            if self.hparams.optim == "SGD":
                ...
                optimizer = SGD(
                    param_groups, momentum=self.hparams.betas[0], lr=lr, weight_decay=0
                )
            ...
            schedulers = [
                {
                    "scheduler": WD_scheduler(
                        optimizer,
                        mode="min",
                        factor=self.hparams.scheduler_decay,
                        trigger_threshold=self.hparams.scheduler_threshold,
                        counter_threshold=self.hparams.counter_threshold[0],
                        min_lr=self.hparams.weight_decay_min,
                        eps=0,
                    ),
                    "monitor": "scheduler/criterion wd",
                    "interval": "step",
                    "frequency": 1,
                    "name": "monitor",
                },
            ...
            schedulers = [
                config.__dict__ for config in self.trainer.lr_scheduler_configs
            ]
    ...
    def on_after_backward(self, *args):
        ...
        if self.hparams.record_wg_hist > 0:
            record_condition = not (
                self.current_step % self.hparams.record_wg_hist
            )  # 50)
            if record_condition:
    ...
    def _step(self, data, batch_idx: int, train_or_test: str = "train"):
        ...
        other_losses = {}
        loss_reconst, acc, out, other_outputs = self.model.evaluate(
            data, train_or_test=train_or_test
        )
        ...
        info = {
            "loss/reconst": loss_reconst.detach(),
            "batch": batch,
        }
        if acc is not None:
    ...
    def training_step(self, data, batch_idx):
        ...
        :param batch_idx: which batch this is in the epoch.
        """
        loss_reconst, info, other_losses = self._step(
            data=data, batch_idx=batch_idx, train_or_test="train"
        )

        if "regularization" in other_losses.keys():
            reg_loss = self.current_weight_decay * other_losses["regularization"]
            reg_loss_1 = (
                self.hparams.conv_weight_decay
                * self.current_wd_coeff
        ...
        self.log(
            "scheduler/criterion wd", self.scheduler_criterion[0], prog_bar=False
        )  # True)
        self.log(
            "scheduler/criterion lr", self.scheduler_criterion[-1], prog_bar=False
        )  # True)
    ...
    def validation_step(self, data, batch_idx):
        with torch.no_grad():
            loss_main, info, other_losses = self._step(
                data=data, batch_idx=batch_idx, train_or_test="val"
            )
