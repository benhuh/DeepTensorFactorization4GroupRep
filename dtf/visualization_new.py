...
def check_model(model, datamodule, n_vectors):
    """
    ...
    if len(model.model.net_Weight.shape) == 2:
        trained = torch.einsum(
            "ijk,j->ik", model.model.net_Weight, model.model.conv_weight
        )
        w_data = torch.Tensor(np.arange(6) / 100)
        desired = torch.einsum(
            "ijk,j->ik", datamodule.train_dataset.M.to_dense() + 0.0, w_data
        )
    elif len(model.model.net_Weight.shape) == 3:
        trained = torch.einsum(
            "ijk,jc->ick", model.model.net_Weight, model.model.conv_weight
        )
        torch.manual_seed(2)
        w_data = torch.randn(model.model.net_Weight.shape[1], n_vectors) / np.sqrt(
            model.model.net_Weight.shape[1]
        )
        desired = torch.einsum(
            "ijk,jc->ick", datamodule.train_dataset.M.to_dense() + 0.0, w_data
        )
    else:
...
def plot_heatmaps(trained, desired, opt_V=None):
    ...
    trained: PyTorch tensor of size (D, D, D) or (D, D^2, D) or (D, D)
        Learned products.
    desired: PyTorch tensor of size (D, D, D) or (D, D^2, D) or (D, D)
        Desired products.
    opt_V: PyTorch tensor of size (D, D) or (D^2, D^2)
        Orthogonal matrix that best aligns the product tensor and the generating data.
    ...
    if len(trained.shape) == 2:
        ...
        if trained.shape[1] != trained.shape[2]:  # FC case
            if opt_V is None:
                fig = plt.figure(figsize=(int(7 * base_w), int(12 * base_h)))
                # w: 1 for annot, 6 for trained
                # h: 1 for each of the 6 products, for each of desired and trained
                gs = gridspec.GridSpec(int(12 * base_h), int(7 * base_w))
            else:
                fig = plt.figure(
                    figsize=(int(11 * base_w), int(12 * base_h))
                )  # 1 for annot, 6 for trained, 2 for opt_V, 2 for opt_V.T @ opt_V
                gs = gridspec.GridSpec(int(12 * base_h), int(11 * base_w))
            p = 6
        else:  # conv case
            if opt_V is None:
                fig = plt.figure(
    ...
    for idx in range(1, p + 1):
        if trained.shape[1] != trained.shape[2]:
            for jdx in range(p):
                ax = plt.subplot(gs[jdx, idx])
                desired_norm = desired[:, jdx * p + idx - 1, :].detach().numpy()
                sns.heatmap(
                    desired_norm,
                    ax=ax,
                    cmap=cmap,
...
def normalize_factor_list(factor_list, ABC0=None, normalize_0=True, normalize_1=True):
    ...
    if (
        len(factor_list.shape) == 5 and factor_list.shape[1] == 3
    ):  # time x ABC: T x 3 x (6,6,6)
    ...
    if normalize_1:
        A0, B0, _ = (
            factor_list[:, 0] if len(factor_list.shape) == 4 else factor_list[:, -1, 0]
        )  # 0th element and last time_step
...
def get_loss_fn(loss_type):
    ...
    loss_dict = dict(
        ...
        regular_inv=regular_loss,
        sparse=sparse_loss,
        sparse_inv=sparse_loss,
        block_diag=offdiagonal_loss,
...
def optimize_T(
    ...
    loss_type: str
        ...
        In most cases, 'inv' will have better performance.
    lr: float
        Learning rate for gradient descent.
    steps: int
        Number of steps to take in gradient descent.
    reg_coeff: float
        Regularization coefficient for the loss. Enforces orthogonality of V.
...
def optimize_V(
    ...
    for _ in range(steps):
        optim.zero_grad()
        ABC_ = diagonalize(
            ABC, V, loss_type, fit_index=fit_index
        )  # ABC_ = diagonalize(ABC, V, loss_type='sparse', fit_index=fit_index)
        loss = (
            loss_fn(ABC_, M)
...
def get_regular_representation(
    model_or_factor_list,
    datamodule,
    V=None,
    plot_flag=False,
    steps=500,
    lr=1,
    loss_type="block_diag",
    trans_list=[0, 1],
    anti_group=False,
    normalize_0=True,
    normalize_1=True,
    ...
    **kwargs,
    ...
    if M_factors is not None:
        ...
        M = (
            datamodule.train_dataset.M.permute(1, 2, 0).to_dense() + 0
            if hasattr(datamodule, "train_dataset")
            else datamodule
        )  # cab -> abc  (for addition / sym)
        if anti_group:
    ...
    if loss_type is None:
        ...
        factor_list = normalize_factor_list(
            factor_list, normalize_0=normalize_0, normalize_1=normalize_1
        )

    for i in trans_list:
        factor_list[i] = factor_list[i].permute(
            0, 2, 1
        )  # factor_list = [A.permute(0,2,1),B.permute(0,2,1),C]     # Huh? Transpose A,B
...
def optimize_XYZ(
    ABC, XYZ, M, lr, loss_type, steps=100, reg_coeff=0.0, fit_index=None, loss_all=None
):
    loss_all = loss_all or []
    ...
    for _ in range(steps):
        ...
        if reg_coeff > 0:
            ...
            for X in XYZ:
                loss += (
                    reg_coeff
                    * (X.T @ X - torch.eye(X.shape[0]).to(X.device)).pow(2).mean()
                )
...
def get_regular_representation_XYZ(
    model_or_factor_list,
    datamodule=None,
    normalize_1=None,
    XYZ=None,
    ...
    **kwargs,
    ...
    factor_list = normalize_factor_list(
        factor_list0, normalize_0=True, normalize_1=normalize_1
    )
    ...
    if datamodule is None:
        M_factors = None
    elif isinstance(datamodule, LightningDataModule) and hasattr(
    ...
    if XYZ is not None:
        ...
        XYZ, ABC_, loss_all = optimize_XYZ(
            ABC,
            XYZ,
            M,
            lr,
        ...
        if plot_flag:
            fig, axes = plt.subplots(
                1, 2, figsize=(3 * 2, 2), sharex=False, sharey=False
            )
            axes[0].semilogy(loss_all, "-")
            plt.show()
...
def process_train_data_and_netW_hist(netW_hist, datamodule, idx_order):  #'abc'):
    ...
    if idx_order == "abc":
        ...
        if len(netW_hist.shape) == 3:
            netW_hist = netW_hist.permute(
                0, 2, 1
            )  # cab -> cba  # so that y-axis: a, x-axis: b
        elif len(netW_hist.shape) == 4:
            netW_hist = netW_hist.permute(
                0, 1, 3, 2
            )  # t cab -> t cba  # so that y-axis: a, x-axis: b
        else:
            raise ValueError
...
def show_netWeight_hist(
    ...
    **kwargs,
    ...
    netW_hist, train_data, idx_name = process_train_data_and_netW_hist(
        netW_hist, datamodule, idx_order
    )
    ...
    if animate:
        ani = plot_tensor_slices_animation(
            netW_hist,
            idx_name_rows=None,
            idx_name_cols=idx_name,
            t0=t0,
            train_data=train_data,
            transpose=False,
            log_dir=log_dir,
            save_name_extra=save_name_extra,
            **kwargs,
        )
        return ani
    else:
...
def show_grad_hist(model, datamodule, idx_order="abc", **kwargs):
    ...
    ABC_hist = show_sparse_hist(model, datamodule, t_ref=kwargs.get("t_ref", -1))[0]
    ABC_hist = ABC_hist / (ABC_hist.shape[2]) ** (0.5)

    # ABC_hist.requires_grad = True
    A_hist, B_hist, C_hist = ABC_hist.permute(1, 0, 2, 3, 4)
    netW_hist = torch.einsum(
        "taij, tbjk, tcki->tcab", A_hist, B_hist, C_hist
    ) * math.sqrt(A_hist.shape[1])
    ...
    mask = torch.sparse_coo_tensor(
        ab.T, torch.ones(ab.shape[0]), size=(N,) * ab.shape[1]
    ).to_dense()
...
def show_sparse_hist(
    model,
    datamodule,
    datamodule_regular=None,
    which_tensor=[0, 1, 2],
    factor_list=None,
    t_ref=None,
    V=None,
    ...
    fit_index=None,
    **kwargs,
):  # , ABC0=None, lr=0.01, reference_step=-1, loss_type='regular', inv_or_trans='inv', plot_flag=False, anti_group=False, trans_list=[]):  #, trans_list=[0,1]
    if hasattr(model, "trainer"):
        print(model.hparams.log_dir)
    factor_list = (
        factor_list or model.model.factor_list
        if t_ref is None
        else model.w_hist[t_ref].detach().cpu()
    )  # reference_step]

    if XYZ is None:
        datamodule_regular = datamodule_regular or datamodule
        if fit_index == None:
            XYZ, ABC_ = get_regular_representation_XYZ(
                factor_list,
                datamodule_regular,
                loss_type=loss_type,
                ...
                XYZ=XYZ,
                **kwargs,
            )  # , plot_flag=plot_flag, lr=lr, loss_type=loss_type, anti_group=anti_group, trans_list=trans_list, fit_index=fit_index) #, inv_or_trans=inv_or_trans)
        else:
            XYZ, ABC_ = get_regular_representation(
                factor_list,
                datamodule_regular,
                loss_type=loss_type,
                normalize_1=normalize_1,
    ...
    ABC_hist = torch.stack(model.w_hist, dim=0).cpu()
    if XYZ is not None:
        ABC_hist_normalized = torch.stack(
            normalize_factor_list(
                ABC_hist, ABC0=None, normalize_0=True, normalize_1=False
            ),
            dim=1,
        )
        if fit_index == None:
            ...
            ABC_hist_ = diagonalize(
                ABC_hist_normalized, XYZ, loss_type=loss_type, fit_index=None
            )
    else:
        ABC_hist_ = ABC_hist
        if new_order is not None:
    ...
    idx_name_all = [
        ...
        "B",
        "C",
    ]  # for cyclic group (addition)        # idx_name = ['e','(1,2)','(2,3)','(1,2,3)','(1,3,2)','(1,3)'] # for sym3_xy
    idx_name = [idx_name_all[i] for i in which_tensor]

    t0 = model.hparams.record_wg_hist

    save_name_extra = kwargs.pop("save_name_extra", None) or (
        "factors" + "_" + loss_type if loss_type is not None else "factors_native"
    )
    log_dir = None  # model.hparams.log_dir
    ...
    if animate:
        if show_netWeight:
            ...
            netW_hist, train_data, netW_idx_name = process_train_data_and_netW_hist(
                netW_hist, datamodule, idx_order="cab"
            )  # ='abc')
        ...
        for which in which_tensor:
            A_hist = ABC_hist_[:, which].permute(
                1, 0, 2, 3
            )  # A_hist = ABC_hist_[:,which].permute(1,0,3,2)
            plot_tensor_slices2(
                A_hist,
                modify_dim=-1,
                idx_name_rows=idx_name_all[which],
                idx_name_cols=["t="],
                t0=t0,
                train_data=None,
                log_dir=log_dir,
...
def show_eig_hist(
    ...
    **kwargs,
    ...
    ABC_hist = torch.stack(model.w_hist, dim=0).cpu()
    ABC_hist_normalized = torch.stack(
        normalize_factor_list(ABC_hist, ABC0=None, normalize_0=True, normalize_1=True),
        dim=0,
    )
    ...
    title_cols, title_rows = get_titles_list(
        fig_size=(1, l),
        idx_name_rows=None,
        idx_name_cols=idx_name_cols,
        skip=skip,
        t0=t0,
    )
    ...
    for i, ax in enumerate(axes):
        ...
        if not animate_flag:
            ax.set_prop_cycle(
                "color", plt.cm.Spectral(np.linspace(0, 1, len(A_hist_ev)))
            )
...
def get_titles_list(
    fig_size, idx_name_rows=None, idx_name_cols=None, skip=1, t0=1, t_init=0
):
        ...
        if idx_names is None:
            ...
            for idx_name in idx_names:
                ...
                if idx_name.endswith("="):  # time index
                    ...
                    titles = [
                        f"${{{idx_name}}}_{{{i}}}$" for i in range(L)
                    ]  # titles = [f'{idx_name} = {i}' for i in range(L)]
...
def pre_process_tensor(
    T, show_num=[0], skip=1, skip_dim=1, t_init=0, z_pow=1, softmax_dim=None
):
...
def plot_tensor_slices2(
    ...
    **kwargs,
    ...
    )  # time dimension = 1 (skip_dim)
    T, fig_size_orig, fig_size, wrap_around_flag = reshape_T(
        T, modify_dim, M_lim
    )  # fig_size_orig = get_fig_size(T.shape)
    title_cols, title_rows = get_titles_list(
        fig_size_orig,
        idx_name_rows=idx_name_rows,
        idx_name_cols=idx_name_cols,
        skip=skip,
        t0=t0,
        t_init=t_init,
    ...
    for i, row in enumerate(axes):
        for j, ax in enumerate(row):
            ...
            if abc is not None:
                if a_max_flag:
                    i_ = (
                        i + j * axes.shape[0] if abc_dim == 0 else j + i * axes.shape[1]
                    )
                else:
...
def plot_tensor_slices_animation(
    T,
    idx_name_rows=None,
    idx_name_cols=None,
    skip=1,
    ...
    **kwargs,
    ...
    T = pre_process_tensor(
        T, skip=skip, skip_dim=0, z_pow=z_pow, softmax_dim=softmax_dim
    )  # time dimension = 0 (skip_dim)
    T, fig_size_orig, fig_size, wrap_around_flag = reshape_T(
        T, modify_dim=1, M_lim=M_lim
    )  # fig_size_orig = get_fig_size(T.shape[1:])
    title_cols, title_rows = get_titles_list(
        fig_size_orig,
        idx_name_rows=idx_name_rows,
        idx_name_cols=idx_name_cols,
    ...
    ani = animation.FuncAnimation(
        fig, animate, frames=len(T), interval=150, blit=False, repeat_delay=3000
    )
