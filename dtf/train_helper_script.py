...
def get_args(**kwargs):
    default_args = dict(
        ...
        weight_decay_min=0,
        conv_weight_decay=0.1,  # should be 0.1
        lr=1 / 2,
        momentum=0.9,
...
def plot_all(model, skip_list=None, **kwargs):
    ...
    plot_scalars(
        model.hparams.log_dir, skip_list=skip_list, **kwargs
    )  # , 'sing_val j', 'sing_val k'])#, save_name = "grokking_in_matrix_completion3")


def run_exp(
    train_frac, extra_args_str=None, train_flag=True, **kwargs
):  # , model0=None
    ...
    if train_flag:
        ...
        plot_all(
            model
        )  #     plot_scalars(trainer.logger.log_dir, skip_list=['monitor',pl 'norm'])
