...
def train(
    ...
    out = run_exp(
        ...
        loss_fn=loss_fn,
        task_name=task_name,
        tensor_width=tensor_width or 6,
        lr=lr,
        momentum=momentum,
...
def generate_figures(
    ...
    show_steps=4,
    t_init=0,
    plot_all_weights=False,
    normalize_later=False,
    ABC_or_A="A",
):
    ...
    if plot_all_weights:
        ...
        if ABC_or_A == "ABC":
            plot_tensor_slices2(
                model.model.factor_list,
                idx_name_cols=["A", "B", "C"],
                save_name=save_name,
                save_name_extra="ABC_raw",
            )
        else:
            plot_tensor_slices2(
                model.model.factor_list[0],
                idx_name_cols=["A"],
                M_lim=8,
                save_name=save_name,
                save_name_extra="A_raw",
            )
    ...
    if save_name is not None and ("noReg" in save_name):  # or 'L2' in save_name):
        ...
        if plot_all_weights:
            if ABC_or_A == "ABC":
                plot_tensor_slices2(
                    normalize_factor_list(model.model.factor_list),
                    idx_name_cols=["A", "B", "C"],
                    save_name=save_name,
                    save_name_extra="ABC_normalized",
                )
            else:
                plot_tensor_slices2(
                    normalize_factor_list(model.model.factor_list)[0],
                    idx_name_cols=["A"],
                    M_lim=8,
                    loss_type=sparsity_type,
                    save_name=save_name,
                    save_name_extra=f"A_normalized",
                )

    XYZ, factor_list_sparse = get_regular_representation_XYZ(
        model,
        datamodule,
        plot_flag=False,
        XYZ=XYZ_prev,
    ...
    if plot_all_weights:
        if ABC_or_A == "ABC":
            if normalize_later:
                plot_tensor_slices2(
                    normalize_factor_list(factor_list_sparse),
                    idx_name_cols=["A", "B", "C"],
                    loss_type=sparsity_type,
                    save_name=save_name,
                    save_name_extra=f"ABC_{sparsity_type_str}_normalized",
                )
            else:
                plot_tensor_slices2(
                    factor_list_sparse,
                    ...
                    loss_type=sparsity_type,
                    save_name=save_name,
                    save_name_extra=f"ABC_{sparsity_type_str}",
                )
        else:
            plot_tensor_slices2(
                factor_list_sparse[0],
                idx_name_cols=["A"],
                M_lim=8,
                loss_type=sparsity_type,
                save_name=save_name,
                save_name_extra=f"A_{sparsity_type_str}",
            )

    A_hist_, XYZ_ = show_sparse_hist(
        model,
        datamodule,
        which_tensor=which_tensor,
        new_order=None,
        XYZ=XYZ,
        animate=False,
        skip=skip,
        ...
        loss_type=sparsity_type,
        save_name=save_name,
        save_name_extra=f"A_hist_{sparsity_type_str}",
    )
    show_netWeight_hist(
        model,
        datamodule,
